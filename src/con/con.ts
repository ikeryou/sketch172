...
export class Con extends Canvas {
  ...
  protected _update(): void {
    ...
    const block = this._item[0].itemSize.x * 1.1
    this._item.forEach((val) => {
      val.position.x = val.itemPos.y * block - (block * this._line * 0.5 + block * 0.5)
      val.position.z = val.itemPos.x * block - (block * this._line * 0.5 + block * 0.5)
      val.position.y = -block
      // val.position.z = block * (val.itemPos.y % 2 == 0 ? 1 : -1)
